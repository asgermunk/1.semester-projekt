...
Promise.all([
  ...
  function mouseClickMap(d) {
    ...
    const scaleFactorMap =
      0.8 /
      Math.max(bboxWidthMap / widthMap, bboxHeightMap / heightMap);
    ...
    const xMap =
      widthMap / 2 - scaleFactorMap * centroidMap[0];
    const yMap =
      heightMap / 2 - scaleFactorMap * centroidMap[1];
...
d3.csv(
  ...
  var x = d3
    ...
    .range([0, 2 * Math.PI])
    .align(0)
    .domain(dataChart.map(function (d) {
      return d.Country;
    }));
